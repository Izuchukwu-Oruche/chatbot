...
def handle_text(from_id: str, text: str):
    ...
    if _session_age_seconds(sess) > IDLE_RESET_SECONDS:
        sess = {"wa_id": from_id, "state":"idle", "intent":"unknown", "lang": sess.get("lang","auto"), "slots":{}, "missing_slots": []}
        save_session(sess)

    # Use auto language on first turn; afterwards, stick to session language
    preferred = "auto" if (sess.get("intent","unknown")=="unknown" and not (sess.get("slots") or {})) else (sess.get("lang") or "auto")

    parsed = llm_parse(text, prev_intent=sess.get("intent","unknown"), prev_slots=sess.get("slots") or {}, preferred_lang=preferred)
